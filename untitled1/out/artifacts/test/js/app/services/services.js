...
angular.module('app.services', [])
  .service("skillService", function($http, $log, $q){
    return {
      getSkillList : function(skill){
        var deferred = $q.defer();
        $http.post('/hb/Skill/getSkills.json', skill).then(function(response){
  ...
  .factory('httpInterceptor', ['$q', '$location', function ($q, $location) {
      return {
        ...
        request: function (config) {
            ...
            //$location.path("/smbr/wolf-login.html");
        ...
        responseError: function (rejection) {
            // 对失败的响应进行处理
            // 例如 统一的弹窗提示
            //console.log($location.protocol() + "://" + $location.host() + ":" + $location.port() + "/hbr/index.html");
            // window.location.href = $location.protocol() + "://" + $location.host() + ":" + $location.port() + "/hbr/index.html";
            //console.log(rejection);
            if(rejection.status == 600){
                window.location.href = "/smbr/wolf-login.html";
            }
            return $q.reject(rejection);
            // window.location.href = "/smbr/wolf-login.html";
